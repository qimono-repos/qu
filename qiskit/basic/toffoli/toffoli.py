    ...
    for c1 in (0, 1):
        for c0 in (0, 1):
            for t in (0, 1):
                ...
                bits = next(iter(qk.quantum_info.Statevector.from_instruction(qc).to_dict()))
...
def superposition_controls() -> None:
    ...
    qc = qk.QuantumCircuit(3, 3, name="toffoli_superposed")
    qc.h(1)
    qc.h(2)
    qc.ccx(2, 1, 0)
...
def main() -> None:
    demo = qk.QuantumCircuit(3, name="toffoli")
    demo.ccx(2, 1, 0)
